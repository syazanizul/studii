...
//            $total_monthly = 0;
//        }
        dd(DB::table('count_attempt')->where('creator', Auth::user()->id)->whereMonth('created_at', Carbon::now()->month)->get());
        if(DB::table('count_attempt')->where('creator', Auth::user()->id)->whereDate('created_at', Carbon::now()->month)->get() -> isNotEmpty())    {
